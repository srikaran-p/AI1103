...
\begin{multline}
    N\brak{\det{M} \neq 0} = \brak{2^n - 1}\brak{2^n - 2}\brak{2^n - 2^2}\\ \dots \brak{2^n - 2^{n-1}}
\end{multline}
Matrices with $\det{M} < 0$ are the matrices resulting from swapping of first two rows of matrices with $\det{M} > 0$.

\begin{align}
    N\brak{\det{M} > 0} &= N\brak{\det{M} < 0}\\
    &= \frac{1}{2}N\brak{\det{M} \neq 0}\\
    &= \frac{1}{2}\prod_{k=0}^{n-1}\brak{2^n - 2^k}\\
    \pr{\det{M} > 0} &= \frac{N\brak{\det{M} > 0}}{2^{n^2}}\\
